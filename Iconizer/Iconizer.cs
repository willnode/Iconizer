...
public class Utility
{
    ...
    public static Icon GetNicestIcon (Icon[] icons)
    {
        var nicest = icons[0];
        var nicestDepth = TsudaKageyu.IconUtil.GetBitCount(nicest);
        for (int i = 1; i < icons.Length; i++)
        {
            var icon = icons[i];
            var bit = TsudaKageyu.IconUtil.GetBitCount(icon);
            if (!(icon.Width < nicest.Width && icon.Height < nicest.Height) && bit >= nicestDepth)
            {
                nicest = icon;
                nicestDepth = bit;
            }
